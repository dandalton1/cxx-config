...
// CXXConfigExportImport::CXXConfigExportImport() { this->parent = nullptr; }

// CXXConfigExportImport::CXXConfigExportImport(const CXXConfigExportImport &other) : CXXConfigExportImport() {
// 	// TODO add copy
// 	*this = other;
// }

// bool CXXConfigExportImport::isSet(const std::string &key) {
...
// CXXConfigExportImport &CXXConfigExportImport::getSubConfig(const std::string &key) {
// 	std::map<std::string, CXXConfigExportImport *>::iterator conIt;
...
// 		CXXConfigExportImport *_config = getSuperInstance();
...
// void CXXConfigExportImport::printTable() const { this->printTable(SystemInfo::getStdOut()); }

// void CXXConfigExportImport::printTable(Ref<IO> &io) const {
...
// 	/*  Check the depth.    */
// 	const CXXConfigExportImport *config = this;
// 	int depth = 0;
// 	while (config) {
...
// 	std::map<std::string, CXXConfigExportImport *>::const_iterator conIT;
// 	conIT = this->config.begin();
// 	for (; conIT != this->config.cend(); conIT++) {
// 		const std::string &name = (*conIT).first;
// 		IOUtil::format(io, "----- %s ----\n", name.c_str());
// 		const CXXConfigExportImport *_config = (*conIT).second;
...
static CXXConfigExportImport::ConfigFormat predictConfigExtFormat(const char *path) {
	CXXConfigExportImport::ConfigFormat format = CXXConfigExportImport::JSON;
	if (format == CXXConfigExportImport::ConfigFormat::Unknown) {
		// throw InvalidArgumentException("Could not predict file format for: {}", path);
	}
	return format;
}

void CXXConfigExportImport::save(IConfig &config, const char *path, CXXConfigExportImport::ConfigFormat format) {
	// if (!io->isWriteable()) {
	//	throw std::runtime_error(fmt::format("IO object %s is not writable.", io->getName().c_str()));
	//}
	if (format == ConfigFormat::Unknown) {
		format = predictConfigExtFormat(path);
	}
...
void CXXConfigExportImport::save_xml(Config& config, Ref<IO> &io) {
	int rc;
	...
	for (; conIT != this->config.cend(); conIT++) {
		const std::string &name = (*conIT).first;
		CXXConfigExportImport *_config = (CXXConfigExportImport *)(*conIT).second;
		_config->save_xml(io);
	}
	/*  */
...
void CXXConfigExportImport::save_yaml(Ref<IO> &io) {
...
void CXXConfigExportImport::save_json_recursive_config(const CXXConfigExportImport *config, struct json_object *root) {
	std::map<std::string, CXXConfigExportImport *>::const_iterator conIT;
	std::map<std::string, int>::const_iterator iti;
	std::map<std::string, float>::const_iterator fti;
	std::map<std::string, std::string>::const_iterator sti;
...
void CXXConfigExportImport::save_json(Ref<IO> &io) {
...
void CXXConfigExportImport::parseConfigFile(Ref<IO> &io, ConfigFormat format) {
	if (!io->isReadable())
...
void CXXConfigExportImport::parse_xml(Ref<IO> &io) {
	xmlDoc *document;
...
void CXXConfigExportImport::parse_yaml(Ref<IO> &io) {
	yaml_parser_t parser;
	yaml_token_t token;
	char *data;
	if (!yaml_parser_initialize(&parser))
...
void CXXConfigExportImport::parse_json_recursive_config(CXXConfigExportImport *config, struct json_object *root) {
	int val_type, i;
	...
	json_object_object_foreach(root, key, value) {
		...
		case json_type_object: {
			...
			CXXConfigExportImport &subConfig = config->getSubConfig(key);
...
void CXXConfigExportImport::parse_json(Ref<IO> &io) {
	/*  */
	char buf[1024 * 4];
	int len;
	/*  */
...
// CXXConfigExportImport::~CXXConfigExportImport() {
// 	/*  Release all resources.  */

// 	/*  */
// 	std::map<std::string, CXXConfigExportImport *>::iterator conIT = this->config.begin();
// 	for (; conIT != this->config.end(); conIT++) {
// 		CXXConfigExportImport *_config = (*conIT).second;
...
// CXXConfigExportImport *CXXConfigExportImport::getSuperInstance() { return new CXXConfigExportImport(); }
